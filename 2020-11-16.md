...
我们选择了对于程序员最简单的方式：GitHub Pages。GitHub 免费为每个用户账号提供了一个 github.io 的子域名使用，同时支持进行自定义域名的托管。我注意到 plctlab 还没有被人抢注（看来我们的知名度还不够呀笑），于是，锵锵～ plctlab.org 现在成为了我们的英文门户。目前已经有了第一篇英文博客，是来自张尹同学的《Optimze OpenCV for RISC-V》。中文搞已经在知乎上发布过，这次是中翻英。后续，我们也会继续默默的添加新的文章。欢迎感兴趣的小伙伴关注新的仓库。
