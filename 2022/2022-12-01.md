...
时间过得真快，一年又要过去了。而按照现在流行的说法，新冠纪元第4年开始了。没到了一个时间点，人就会自然的回忆和总结过去一个时间段的收获和悲喜。作为一个团队也是如此。我们将在下周举办第四届 PLCT OpenDay 开放日活动。欢迎大家12月9日（周五）来听一听、聊一聊。今年的议程里多了不少新同事，很欣慰中科院软件所在基础开源软件的团队日益壮大。明年还需更加努力。

## 本期亮点

- 中科院软件所PLCT实验室的工程师向 Mozilla 社区提交了 Spidermonkey 的 RISC-V JIT porting 工作，并且已经收到了 JS Team 的接收确认。目前有一些 patch 细节的调整正在进行中，非常有希望在农历春节之前合并进入。再一次欢呼吧， RISC-V 社区的伙伴们！
...
- JAVA 开源工具调研及测试
  - [Linux-RISCV64平台 Java配套开源性能分析工具调研和使能](https://github.com/jvm-profiling-tools/async-profiler/pull/644#issuecomment-1328474224)
...
欢迎陈逸轩同学加入GNU小队，目前正在完成一些基础性的工作，期待她的表现。

本月在RISC-V GNU toolchain会议上讨论了profiles实现方案，并在工具链上实现了支持，正在根据收到的意见修改中：

https://gcc.gnu.org/pipermail/gcc-patches/2022-November/604869.html

协助验证修复了RVV epilogue中的一些问题：
...
- Spidermonkey 正式开始upstream流程。Upstream Url:  https://phabricator.services.mozilla.com/D161986?id=647929
- Patches
...
- 为内部的builtin ABI添加了最大长度上限（暂时使用32位最大长度上限）
...
- 可变SXLEN支持：https://github.com/plctlab/plct-qemu/tree/plct-rv32u-dev
- Zc*扩展更新到v8版本：https://github.com/plctlab/plct-qemu/tree/plct-zce-upstream-v8

## Other Support for RISC-V International

### SAIL/ACT

- CMO ACT支持进行了更新，等待合并：https://github.com/riscv-non-isa/riscv-arch-test/pull/226
