...
删个月，PLCT 正式的成为软件所内部负责 openEuler 在 RISC-V 上适配和优化的团队，开始承担起又一个发行版的支持工作。我们开始了新一轮的招聘，并很快收到了数十位同学的简历（恭喜各位同学加入PLCT实验室成为实习生🎉），并已经让 openEuler 能够在全志哪吒开发板上运行起来（进入了命令行，图形界面还不行）。
